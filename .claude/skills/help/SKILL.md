...
| `/create-feature: <description>` | File a new `FEAT-NNNN` under `features/` (non-rule-linked idea/roadmap entry) |
...
| `/list <type> [--filter ...]` | List artifacts, work items, rules, or templates, with optional property filters |
| `/freeze <item-id\|item-path\|type\|template-name>` | Protect an item, type, or template from being overwritten by `/sync-framework` |
| `/catalyzer <list\|activate\|download\|deactivate\|upgrade\|downgrade>` | Manage plugin installation and activation |
| `/status <artefact-id> <status> [force]` | Update an artifact's `Status` field, refusing invalid values unless `force` is given |
| `/audit <file-name>` | Analyze the change-impact of a specified file across the deployed framework |
...
| Feature | `features/` | `FEAT-NNNN` | Non-rule-linked idea/roadmap entry for possible future work; never carries `Targets`/`Domain`; motivates a `REQ-NNNN` once work starts |
...
Every `ui-`/`br-` rule is its own file under `rules/ui/` or
`rules/business/`, indexed by that type's `ui-rules.md`/`business-rules.md`
and by the global `.catalyst-proj/rules/rules.md` index. ID format
`(ui|br|rr)-(DOMAIN)-(NNN)`, governed by
