...
Per `.catalyst-proj/rules/rules-of-development.md` §3: inspect the
...
1. **Every `BUG-`/`REQ-`/`HK-` item has a non-empty `Targets` field**
   ...
   `ui-`/`br-`/`rr-` scheme) — per `rules-of-development.md` §1. Flag any
...
3. **Every domain in `domains/`** has a corresponding entry in
   ...
   not drift apart (per `rules-of-rules.md` §6 step 3).
4. **Every index file is complete**: `bugs/bugs.md`,
   `requirements/requirements.md`, `house-keeping/house-keeping.md`,
...
7. **No rule marked 🗑 retired is still cited as a live `Targets`** by a
   ...
   `rules-of-rules.md` §4 and `rules-of-development.md` §7 — retired
