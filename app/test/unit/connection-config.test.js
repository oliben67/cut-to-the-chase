...
const { loadConnectionConfig, defaultConfigPath } = require("../../lib/connection-config");
...
test("file with full ssh-tunnel config", () => {
  const p = tmpConfigPath();
  writeConfig(p, { mode: "ssh-tunnel", ssh_target: "deploy@docker-host", ssh_key: "~/.ssh/k", remote_port: 8765 });
  assert.deepEqual(loadConnectionConfig({ env: {}, configPath: p }), {
    mode: "ssh-tunnel", sshTarget: "deploy@docker-host", sshKey: "~/.ssh/k", remotePort: 8765,
  });
});

test("ssh_key defaults to null when omitted", () => {
  const p = tmpConfigPath();
  writeConfig(p, { mode: "ssh-tunnel", ssh_target: "deploy@docker-host", remote_port: 8765 });
...
test("env vars fully override the file", () => {
  const p = tmpConfigPath();
  writeConfig(p, { mode: "ssh-tunnel", ssh_target: "file@host", ssh_key: "file-key", remote_port: 1111 });
  const env = {
    CTTC_MODE: "ssh-tunnel",
    CTTC_SSH_TARGET: "env@host",
    CTTC_SSH_KEY: "env-key",
    CTTC_REMOTE_PORT: "2222",
  };
  assert.deepEqual(loadConnectionConfig({ env, configPath: p }), {
    mode: "ssh-tunnel", sshTarget: "env@host", sshKey: "env-key", remotePort: 2222,
  });
});

test("env can flip an embedded file to ssh-tunnel mode as long as env supplies the rest", () => {
  const p = tmpConfigPath();
  writeConfig(p, { mode: "embedded" });
  const env = { CTTC_MODE: "ssh-tunnel", CTTC_SSH_TARGET: "env@host", CTTC_REMOTE_PORT: "8765" };
  const got = loadConnectionConfig({ env, configPath: p });
  assert.equal(got.mode, "ssh-tunnel");
});

test("missing ssh_target throws a clear error", () => {
  const p = tmpConfigPath();
  writeConfig(p, { mode: "ssh-tunnel", remote_port: 8765 });
...
for (const bad of [undefined, "", "not-a-number", 0, -1, 70000, 3.5]) {
  test(`invalid remote_port (${JSON.stringify(bad)}) throws`, () => {
    const p = tmpConfigPath();
    const cfg = { mode: "ssh-tunnel", ssh_target: "deploy@host" };
